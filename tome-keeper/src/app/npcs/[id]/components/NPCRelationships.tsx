	...
	return (
		<div className="bg-white dark:bg-gray-800 rounded-lg border border-teal-200 dark:border-teal-800/30 shadow-sm overflow-hidden">
			<div className="flex items-center p-3 border-b border-teal-200 dark:border-teal-800/30 bg-teal-50 dark:bg-teal-900/20">
				...
				<h3 className="text-lg font-semibold text-teal-700 dark:text-teal-400">Relationships</h3>
			</div>
			<div className="p-4">
					...
					{Array.isArray(npc.relationships) ? (
						npc.relationships.map((relationship, index) => {
							...
							const relationshipText =
								typeof relationship === "string" ? relationship : relationship.description
							const relationshipId =
								typeof relationship === "string"
									? `rel-${String(relationship).substring(0, 20)}-${index}`
									: relationship.id || `rel-${index}`

							return (
								<li key={relationshipId} className="flex items-start">
									<span className="text-teal-400 mr-2 mt-1">•</span>
									<span className="text-gray-700 dark:text-gray-300">{relationshipText}</span>
								</li>
							)
						})
					) : (
						<li className="flex items-start">
							<span className="text-teal-400 mr-2 mt-1">•</span>
							<span className="text-gray-700 dark:text-gray-300">{npc.relationships}</span>
						</li>
					)}
				</ul>
			</div>
