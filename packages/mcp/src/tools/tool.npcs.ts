...
import { getGeminiEmbedding, getTextForEntity } from "../embeddingUtils"
 ...
 */
export type NpcToolNames = "get_all_npcs" | "get_npc_by_id" | "generate_npc_embedding" | TableTools

const test = createInsertSchema(npcs)
...
export const schemas = {
	...
	manage_npcs: createInsertSchema(npcs, {
		...
		appearance: jsonArray.describe(
			"Physical characteristics, clothing, distinctive features, and overall look in point form",
		),
		...
		knowledge: jsonArray.describe(
			"Specific information, expertise, and secrets this NPC possesses that players might discover",
		),
	...
	manage_npc_factions: createInsertSchema(npcFactions, {
		secrets: jsonArray.describe(
			"Hidden information about their faction involvement, potentially unknown to other faction members",
		),
		...
		loyalty: z
			.number()
			.describe("Numeric rating of dedication to the faction's interests (0-4: none, low, medium, high)"),
		justification: z
			.string()
			.describe("Reason for their allegiance (personal belief, financial gain, blackmail, family ties, etc.)"),
	...
	manage_npc_locations: createInsertSchema(npcLocations, {
		...
		description: jsonArray.describe(
			"Details about how and why the NPC frequents this location and their typical activities there",
		),
	...
	manage_character_relationships: createInsertSchema(characterRelationships, {
		...
		narrativeTensions: jsonArray.describe(
			"Points of friction, disagreement, or dramatic potential between these characters",
		),
		sharedGoals: jsonArray.describe("Common goals that unite these NPCs"),
		relationshipDynamics: jsonArray.describe(
			"How these NPCs typically interact, communicate, and behave around each other",
		),
		...
		strength: z
			.number()
			.describe("Numeric indicator of relationship intensity (1-4: weak, moderate, strong, unbreakable)"),
	})
		.strict()
...
export const npcToolDefinitions: Record<NpcToolNames, ToolDefinition> = {
	...
	generate_npc_embedding: {
		description: "Generate and save the vector embedding for a specific NPC based on their combined text fields.",
		inputSchema: zodToMCP(z.object({ id: z.number().describe("The ID of the NPC to generate the embedding for.") })),
		handler: async (args) => {
			const parseResult = z.object({ id: z.number() }).safeParse(args)
			if (!parseResult.success) {
				logger.error("Invalid arguments for generate_npc_embedding", { errors: parseResult.error.flatten() })
				return {
					isError: true,
			...
			try {
				logger.info(`Generating embedding for NPC ID: ${id}`)

				// 1. Fetch the NPC record
				const npcRecord = await db.query.npcs.findFirst({
					where: eq(npcs.id, id),
				})

				if (!npcRecord) {
					logger.warn(`NPC not found for embedding generation: ${id}`)
					return { isError: true, content: [{ type: "text", text: `NPC with ID ${id} not found.` }] }
				}

				// 2. Generate the combined text
				const combinedText = getTextForEntity("npcs", npcRecord)
				if (!combinedText) {
					logger.warn(`No text generated for NPC ID: ${id}. Skipping embedding.`)
					return {
						content: [{ type: "text", text: `No text content found for NPC ID ${id} to generate embedding.` }],
					}
				}
				...
				await db.update(npcs).set({ embedding: embedding }).where(eq(npcs.id, id))

				logger.info(`Successfully generated and saved embedding for NPC ID: ${id}`)
				return {
					content: [{ type: "text", text: `Embedding generated successfully for NPC ID: ${id}` }],
				}
			} catch (error) {
				logger.error(`Error generating embedding for NPC ID ${id}:`, {
				...
				return {
					...
					content: [
						{
							type: "text",
							text: `Failed to generate embedding for NPC ID ${id}: ${error instanceof Error ? error.message : String(error)}`,
						},
					],
				}
			}
