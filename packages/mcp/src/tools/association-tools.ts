...
export const entityGetters: AssociationGetters = {
	...
	quest_hook_npc_by_id: (id: number) =>
		db.query.questHookNpcs.findFirst({
			where: (questHookNpcs, { eq }) => eq(questHookNpcs.id, id),
			with: {
				npc: true,
				hook: true,
			},
