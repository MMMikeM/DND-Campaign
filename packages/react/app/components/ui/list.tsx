	...
	if (!items || items.length === 0) {
		return (
			<div {...props}>
				...
				<ul className={cn(listVariants({ listStyle, position, spacing, textSize, textColor }), className)}>
			...
			<ul className={cn(listVariants({ listStyle, position, spacing, textSize, textColor }), className)}>
				{displayItems.map((item, index) => (
					<li
						...
						className={textColor === "muted" ? "text-muted-foreground" : ""}
					>
						{item}
					</li>
				))}
				...
				<AnimatePresence>
					{displayMore && (
						<motion.div
							className={cn(listVariants({ listStyle, position, spacing, textSize, textColor }), className)}
							style={{ overflow: "hidden" }}
							initial={maxItems ? "hidden" : "visible"}
							animate={{
								height: "auto",
								transition: {
