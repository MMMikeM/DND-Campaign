...
export default function LocationDisplay({
	...
	const isLocationFile = locationId?.includes("locations");
	...
	const handleTypeChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
		...
		// Check if the new filter results in only one location
		const newFilteredLocations = locationsData?.locations
			? newType === "all"
				? locationsData.locations
				: locationsData.locations.filter(
						(location) => location.type === newType,
					)
			: [];

		// If there's only one location in the filtered results, automatically select it
		if (newFilteredLocations.length === 1) {
			setSelectedLocationId(newFilteredLocations[0].id);
		}
		// If there are multiple results but the current selection isn't in the filtered set
		else if (
			newFilteredLocations.length > 0 &&
			selectedLocationId &&
			!newFilteredLocations.some((loc) => loc.id === selectedLocationId)
		) {
			setSelectedLocationId(newFilteredLocations[0].id);
		}
	};
					...
					<div>
						...
						>
							{filteredLocations.length === 0 ? (
								<option value="">No locations of this type</option>
							) : filteredLocations.length === 1 ? (
								<option value={filteredLocations[0].id}>
									{filteredLocations[0].name} (only option)
								</option>
							) : (
