...
export default async function ContentDisplayServer({
	contentType,
	filename,
}: ContentDisplayServerProps) {
	...
	switch (contentType) {
		...
		case "location": {
			...
			// If it's the locations index file, redirect to the locations page
			if (isLocationsFile) {
				logger.debug.data("Redirecting to locations index", {
					filename: parsedFilename,
				})
				redirect("/locations")
			}
			...
			// Redirect to the location page - the server component will handle finding the right category
			logger.debug.data("Redirecting to location", { locationId })
