...
export default function YamlDisplay({ filename }: YamlDisplayProps) {
	...
	switch (contentType) {
		case "faction": {
			...
			// Check if it's the main factions file first
			const factionFilename = filename?.toLowerCase() || "";
			const isFactionsFile = factionFilename.includes("factions");
			const factionId = isFactionsFile
				? undefined // Don't pass a factionId for the factions file, let the component handle it
				: factionFilename.replace(/\.ya?ml$/, "");
			return <FactionDisplay factionId={factionId} handleRef={handleRef} />;
		}
		case "npc": {
			// Extract NPC ID from filename if possible
			// Check if it's the main NPCs file first
			const npcFilename = filename?.toLowerCase() || "";
			const isNpcsFile = npcFilename.includes("npcs");
			const npcId = isNpcsFile
				? undefined // Don't pass a npcId for the NPCs file, let the component handle it
				: npcFilename.replace(/\.ya?ml$/, "");
			return <NPCDisplay npcId={npcId} handleRef={handleRef} />;
		}
		case "quest": {
