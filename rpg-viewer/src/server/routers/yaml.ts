...
async function getYamlFiles(directory: string): Promise<string[]> {
...
export const yamlRouter = router({
  ...
  getFileList: publicProcedure.query(async () => {
    try {
      ...
      const rootYamlFiles = await getYamlFiles(rootDir);
      const publicYamlFiles = await getYamlFiles(publicDataDir);
